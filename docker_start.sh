...
curl https://raw.githubusercontent.com/TT-IPIP/qd/main/docker/config.template.json -o config/config.json
...
if [ $? -ne 0 ];then
  ...
  if [ $? -ne 0 ];then
    ...
    curl -O https://raw.githubusercontent.com/TT-IPIP/qd/main/docker/docker-compose.yml
    echo "下载 Makefile 文件（可以无视）"
    curl -O https://raw.githubusercontent.com/TT-IPIP/qd/main/docker/Makefile
